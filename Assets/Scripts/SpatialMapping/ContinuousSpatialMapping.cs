...
/// Note that ContinuousSpatialMappingManager is more user-friendly/high-level, designed to hide the complexities of this class. 
/// </summary>
public class ContinuousSpatialMapping
{
    /// <summary>
    /// Submesh created by ContinuousSpatialMapping. The scan is made of multiple chunks. 
    /// </summary>
    public struct Chunk
    {
        /// <summary>
    ...
    {
        /// <summary>
        /// Geometry within 2 meters of the camera will be mapped. 
        /// </summary>
        SUPER_NEAR,
    ...
    /// </summary>
    private ContinuousSpatialMappingHelper spatialMappingHelper;
    ...
    /// </summary>
    public Dictionary<int, ContinuousSpatialMapping.Chunk> Chunks
    {
    ...
    /// </summary>
    public List<ContinuousSpatialMapping.Chunk> ChunkList = new List<ContinuousSpatialMapping.Chunk>();

    /// <summary>
    /// Constructor. Spawns the holder GameObject to hold scanned chunks and the ContinuousSpatialMappingHelper to handle low-level mesh processing. 
    /// </summary>
    /// <param name="transform">Transform of the scene's ContinuousSpatialMappingManager.</param>
    ...
    public ContinuousSpatialMapping(Transform transform, sl.ZEDCamera zedCamera, ZEDManager zedManager, Material material, float timeout, float renderDistance)
    {
        //Instantiate the low-level mesh processing helper. 
        spatialMappingHelper = new ContinuousSpatialMappingHelper(Resources.Load("Materials/SpatialMapping/Mat_ZED_Texture") as Material, material, timeout, renderDistance);
    ...
    /// <summary>
    /// Initializes flags used during scan, tells ContinuousSpatialMappingHelper to activate the ZED SDK's scanning, and 
    ...
    private sl.ERROR_CODE EnableSpatialMapping(RESOLUTION resolutionPreset, RANGE rangePreset, bool isTextured)
    {
        ...
        error = spatialMappingHelper.EnableSpatialMapping(ContinuousSpatialMappingHelper.ConvertResolutionPreset(resolutionPreset), ContinuousSpatialMappingHelper.ConvertRangePreset(rangePreset), isTextured);
    ...
    private void UpdateMeshMainthread(bool spreadUpdateOverTime = true)
    {
        ...
        lock (lockScanning) //Don't update if another thread is accessing. 
        {
            ...
            {
                spatialMappingHelper.CleanUpOldMeshes();
    ...
    /// Functions are usually called from ContinuousSpatialMapping, but buffer data is held within. 
    /// Note that some values are updated directly from the ZED wrapper dll, so such assignments aren't visible in the plugin. 
    /// </summary>
    private class ContinuousSpatialMappingHelper
    {
        /// <summary>
        ...
        /// </summary>
        public Dictionary<int, ContinuousSpatialMapping.Chunk> chunks = new Dictionary<int, ContinuousSpatialMapping.Chunk>(MAX_SUBMESH);
        ...
        public ContinuousSpatialMappingHelper(Material materialTexture, Material materialMesh, float timeout, float renderDistance)
        {
        ...
        {
            if (rangePreset == RANGE.SUPER_NEAR)
            {
                return 0.1f;
            }
            else if (rangePreset == RANGE.NEAR)
            {
                return 3.5f;
            }
        ...
        /// </summary>
        public ContinuousSpatialMapping.Chunk CreateNewMesh(int i, Material meshMat, Transform holder)
        {
            //Initialize the chunk and create a GameObject for it. 
            ContinuousSpatialMapping.Chunk chunk = new ContinuousSpatialMapping.Chunk();
        ...
        /// </summary>
        public void UpdateMeshCollider(List<ContinuousSpatialMapping.Chunk> listMeshes, int startIndex = 0)
        {
            // List<int> idsToDestroy = new List<int>();
            //List of meshes that are too small for colliders and will be destroyed.

            if (listMeshes.Count == 0) return;

            curUpdateIndex = (curUpdateIndex + 1) % listMeshes.Count;

            //Update each mesh with a collider.
            // for (int i = startIndex; i < listMeshes.Count; ++i)
            {
                int i = curUpdateIndex;
                ...
                //     // continue;
                // }
                // else if (Time.time - submesh.lastUpdateTime > timeout)
                // {
                //     idsToDestroy.Add(i);
                // }
        ...
        Dictionary<int, float> lastUpdateTime = new Dictionary<int, float>();
        ...
        {
            ContinuousSpatialMapping.Chunk subMesh;
            ...
            // update the touched time
            lastUpdateTime[updatedIndex] = Time.time;

            Mesh currentMesh = subMesh.mesh;
            ContinuousSpatialMapping.ProceduralMesh dynamicMesh = subMesh.proceduralMesh;
        ...
        public void CleanUpOldMeshes()
        {
            ...
            {
                var index = pair.Key;
                var updateTime = pair.Value;

                if (time - updateTime > timeout)
                {
            ...
            {
                var index = idsToRemove[i];
                GameObject.Destroy(chunks[index].o);
                chunks.Remove(index);
                lastUpdateTime.Remove(index);
            }
        }
